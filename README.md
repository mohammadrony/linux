# Linux Shell and Configuration Scripts

## Learning Tools

- [The Linux Documentation Project](https://tldp.org/)
- [GNU Bash Reference Manual](https://www.gnu.org/software/bash/manual/html_node/index.html)
- [Baeldung on Linux](https://www.baeldung.com/linux/)

## Configurations

- [VMs Network Interface setup](./configurations/1-VMs-network-interface.md)
- [Secondary Storage Management in Linux](./configurations/2-Secondary-storage.md)
- [Mail Transfer Agent setup in Ubuntu](./configurations/3-Postfix-DKIM-setup.md)
- [MantisBT setup with Nginx and MySQL in Ubuntu](./configurations/4-Mantis-Nginx-MySQL.md)
- [Bugzilla setup with Apache and MySQL in Ubuntu](./configurations/5-Bugzilla-Apache-MySQL.md)
- [OpenProject setup with Apache and PostgreSQL in Ubuntu](./configurations/6-OpenProject-Apache-PostgreSQL.md)
- [Upgrade PostgreSQL cluster version](./configurations/7-Postgres-cluster-upgrade.md)
- [Restore PostgreSQL database](./configurations/8-Postgres-database-restore.md)
- [Linux Resource Monitoring](./configurations/9-Resource-monitoring.md)
- [Logrotate config](./configurations/10-Logrotate-config.md)
- [Python setup with pyenv](./configurations/11-Python-with-pyenv.md)
- [Node setup with nvm](./configurations/12-Node-with-nvm.md)
- [Java and Maven setup](./configurations/13-Java-maven.md)
- [Keycloak with Postgresql Database](./configurations/14-Keycloak-Postgres.md)

## Packages

- [System Monitoring](./packages/monitor/)
- [Network tools](./packages/network/)

## Scripts

- [Cleanup](./scripts/1-cleanup/)
- [File size](./scripts/2-file-size/)
- [PDF](./scripts/3-pdf/)
- [Image](./scripts/4-image/)
- [Video](./scripts/5-video/)
- [Online data](./scripts/6-online-data/)
- [Monitoring](./scripts/7-monitoring/)

## Commands

- [ab](./commands/ab/)
- [curl](./commands/curl/)
- [find](./commands/find/)
- [read](./commands/read/)
- [redirect](./commands/redirect/)
- [sed](./commands/sed/)
- [xargs](./commands/xargs/)

## Linux

### Administrative

- [Resource Monitoring](./Linux/admin/1-Resource-monitoring.md)
- [User Management](./Linux/admin/2-User-management.md)
- [Service Management](./Linux/admin/3-Service-management.md)

### Networking

- [Monitoring Network Connections](./Linux/network/1-Monitor-connections.md)

### Basics

- [Use of Single Quotation vs Double Quotation](./Linux/basic/1-Single-vs-double-quotation.md)
- [Track Working Directory](./Linux/basic/2-Track-working-directory.md)
- [File Management Commands](./Linux/basic/3-File-management.md)
- [Ubuntu Firewall Configuration](./Linux/basic/4-Ubuntu-firewall.md)
- [File Archive with tar](./Linux/basic/5-File-archive.md)

### Core concepts

- [Linux core concepts for troubleshooting](./Linux/core/1-Linux-core-concept.md)

### Remote connection

- [Configure SSH Connection without password](./Linux/ssh/1-Connect-SSH-password-less.md)
- [Setup Ansible control node and managed node in CentOS-9](./Linux/ssh/2-Ansible-setup-by-SSH-CentOS-9.md)

## Git

### Using Git Commands

- [Basics of Git](./Git/1-Basics.md)
- [Pull Changes in Local Repository](./Git/2-Pull-changes.md)
- [Branches in Git](./Git/3-Branches.md)
- [Commit History of Repository](./Git/4-Commit-history.md)
- [Save changes and Undo edits with Git](./Git/5-Stash-changes.md)
