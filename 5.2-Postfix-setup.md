# Setting Mail server in Ubuntu 22

## Initial server setup

### Set server domain name

```bash
sudo apt -y update; sudo apt -y upgrade
sudo echo 'dev.dsinnovators.com' > /etc/hostname
sudo sed -i 's/\(127.0.0.1\).*/\1 dev.dsinnovators.com/' /etc/hosts
sudo timedatectl set-timezone Asia/Dhaka
sudo reboot now
```

## Setup Mail Transfer Agent

### Install required packages

```bash
sudo apt install -y mailutils
sudo apt install -y postfix
```

### Update Postfix configuration

```bash
sudo dpkg-reconfigure postfix
> Internet Site
> System mail name: dev.dsinnovators.com
> Recipient for root: <enter>
> Other destinations to accept mail: dev.dsinnovators.com, localhost.dsinnovators.com, localhost
> Force synchronous updates: <No>
> Local networks: 127.0.0.0/8 [::ffff:127.0.0.0]/104 [::1]/128
> Mailbox size limit: 0
> Local address extension character: +
> Internet protocols to use: all
```

Custom configuration

```bash
sudo vi /etc/postfix/main.cf

# Update
home_mailbox = Maildir/
inet_interfaces = loopback-only
masquerade_domains = dev.dsinnovators.com
```

### Reload Postfix

```bash
sudo systemctl restart postfix
postconf -n
```

### Verify server connection

```bash
telnet smtp.google.com 25
> ehlo smtp.google.com
```

## Setup DKIM key

### Install DKIM packages

```bash
sudo apt install -y opendkim opendkim-tools
```

### Add postfix user to opendkim group

```bash
sudo usermod -G opendkim postfix
```

### Setup opendkim key directory

```bash
sudo mkdir -p /etc/opendkim/keys
sudo chown -R opendkim:opendkim /etc/opendkim
sudo chmod  744 /etc/opendkim/keys
```

### Generate OpenDKIM key

```bash
sudo mkdir /etc/opendkim/keys/dev.dsinnovators.com
sudo opendkim-genkey -b 2048 -d dev.dsinnovators.com -D /etc/opendkim/keys/dev.dsinnovators.com -s default -v
sudo chown opendkim:opendkim /etc/opendkim/keys/dev.dsinnovators.com/default.private
```

### Update DKIM key in DNS record

```bash
sudo cat /etc/opendkim/keys/dev.dsinnovators.com/default.txt

# Output
default._domainkey IN TXT ("v=DKIM1; h=sha256; k=rsa; p=Encrypted_key");
```

### Verify the DNS record

```bash
sudo opendkim-testkey -d dev.dsinnovators.com -s default -vvv
# Or
sudo opendkim-testkey -d dsinnovators.com -s default -vvv
```

### Update DKIM config

```bash
sudo vi /etc/opendkim.conf

# Update
LogWhy                  yes
Mode                    sv
SubDomains              no

# Add
AutoRestart             yes
AutoRestartRate   	    10/1M
Background              yes
DNSTimeout              5
SignatureAlgorithm      rsa-sha256

# End of the file
KeyTable                refile:/etc/opendkim/key.table
SigningTable            refile:/etc/opendkim/signing.table
ExternalIgnoreList      /etc/opendkim/trusted.hosts
InternalHosts           /etc/opendkim/trusted.hosts
```

OR

```bash
sudo sed -i 's/^#\(LogWhy\s*\)no/\1yes/' /etc/opendkim.conf
sudo sed -i 's/^#\(Mode\s*sv\)/\1/' /etc/opendkim.conf
sudo sed -i 's/^#\(SubDomains\s*no\)/\1/' /etc/opendkim.conf

sudo sed -i '18i AutoRestart\t\tyes\nAutoRestartRate\t\t10/1M\nBackground\t\tyes\nDNSTimeout\t\t5\nSignatureAlgorithm\trsa-sha256\n' /etc/opendkim.conf
sudo sed -i '$ a KeyTable\t\trefile:/etc/opendkim/key.table\nSigningTable\t\trefile:/etc/opendkim/signing.table\nExternalIgnoreList\t/etc/opendkim/trusted.hosts\nInternalHosts\t\t/etc/opendkim/trusted.hosts' /etc/opendkim.conf
```

### Add new files for DKIM

```bash
sudo vi /etc/opendkim/signing.table

# Add
*@dev.dsinnovators.com		default._domainkey.dev.dsinnovators.com
*@*.dev.dsinnovators.com	default._domainkey.dev.dsinnovators.com
```

OR

```bash
sudo touch /etc/opendkim/signing.table

sudo awk -i inplace 'BEGINFILE {print "*@dev.dsinnovators.com\t\tdefault._domainkey.dev.dsinnovators.com\n*@*.dev.dsinnovators.com\tdefault._domainkey.dev.dsinnovators.com"}' /etc/opendkim/signing.table

cat /etc/opendkim/signing.table
```

#### Update Key table

```bash
sudo vi /etc/opendkim/key.table

# Add
default._domainkey.dev.dsinnovators.com  dev.dsinnovators.com:default:/etc/opendkim/keys/dev.dsinnovators.com/default.private
```

Or

```bash
sudo touch /etc/opendkim/key.table

sudo awk -i inplace 'BEGINFILE {print "default._domainkey.dev.dsinnovators.com\tdev.dsinnovators.com:default:/etc/opendkim/keys/dev.dsinnovators.com/default.private"}' /etc/opendkim/key.table

cat /etc/opendkim/key.table
```

#### Update hosts

```bash
sudo vi /etc/opendkim/trusted.hosts

# Add
127.0.0.1
localhost
.dev.dsinnovators.com

```

Or

```bash
sudo touch /etc/opendkim/trusted.hosts

sudo awk -i inplace 'BEGINFILE {print "127.0.0.1\nlocalhost\n\n.dev.dsinnovators.com"}' /etc/opendkim/trusted.hosts

cat /etc/opendkim/trusted.hosts
```

#### Restart OpenDKIM

```bash
sudo systemctl restart opendkim
```

### Configure Postfix With OpenDKIM


#### Update OpenDKIM socket file

```bash
sudo mkdir /var/spool/postfix/opendkim
sudo chown opendkim:postfix /var/spool/postfix/opendkim
```

#### Update configuration file

```bash
sudo vi /etc/opendkim.conf

# Update
Socket    local:/var/spool/postfix/opendkim/opendkim.sock
```

Or

```bash
sudo sed -i 's/\(Socket\s*local:\/run\/opendkim\/opendkim.sock\)/#\1/' /etc/opendkim.conf
sudo sed -i 's/#\(Socket\s*local:\/var\/spool\/postfix\/opendkim\/opendkim.sock\)/\1/' /etc/opendkim.conf
```

#### Update OpenDKIM configuration

```bash
sudo vi /etc/default/opendkim

# Update
SOCKET="local:/var/spool/postfix/opendkim/opendkim.sock"
```

Or

```bash
sudo sed -i 's/\(^SOCKET=local:$RUNDIR\/opendkim.sock\)/#\1/' /etc/default/opendkim
sudo sed -i '20a SOCKET=local:/var/spool/postfix/opendkim/opendkim.sock' /etc/default/opendkim

```

#### Update Postfix configuration

```bash
sudo vi /etc/postfix/main.cf

# Add
# Milter configuration
milter_default_action = accept
milter_protocol = 6
smtpd_milters = local:opendkim/opendkim.sock
non_smtpd_milters = $smtpd_milters
```

Or

```bash
sudo sed -i '$ a # Milter configuration\nmilter_default_action = accept\nmilter_protocol = 6\nsmtpd_milters = local:opendkim/opendkim.sock\nnon_smtpd_milters = $smtpd_milters' /etc/postfix/main.cf
```

#### Restart services

```bash
sudo systemctl restart opendkim
sudo systemctl restart postfix
```

## Send Email

```bash
echo "Hello, from Rony" | mail -s "This is a test mail" <user>@yahoo.com
```

Thank you.
